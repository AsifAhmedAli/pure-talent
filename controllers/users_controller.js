...
const new_user = async (req, res) => {
  ...
  conn.query(checkUserQuery, [email], (err, result) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
      ...
      conn.query(insertUserQuery, [name, company, hashedPassword, email], (err, result) => {
        if (err) {
          console.error(err);
          return res.status(500).json({ error: "Internal server error" });
        }
        ...
        const mailOptions = {
          ...
          html: `Please click this link to confirm your email: <a href="${process.env.SITE_URL}/verify-email/${token}">Verify Email</a>`,
        };
...
const user_login = async (req, res) => {
    ...
    conn.query(checkUserQuery, [email], (err, result) => {
      ...
      if (result.length === 0) {
        return res.status(401).json({ error: "you are not registered, please register first" });
      }
      const user = result[0];
      // Check if user has verified their email
      if (user.verified !== 1) {
        return res.status(401).json({ error: "Please verify your email address from you email" });
      }
...
const send_verification_code = async (req, res) => {
    ...
    conn.query(checkUserQuery, [current_email], (err, result) => {
      ...
      const verification_code = randomstring.generate({ length: 6, charset: "numeric" });
      ...
      conn.query(insertVerificationCodeQuery, [current_email, verification_code], (err, result) => {
        if (err) {
          console.error(err);
          return res.status(500).json({ error: "Internal server error" });
        }
...
const change_email = async (req, res) => {
    const { current_email, verification_code, new_email } = req.body;
  
    // Check if all required fields are present
    if (!verification_code || !new_email) {
      return res.status(400).json({ error: "All fields are required" });
    }
  
    // Check if new email address is different from current email address
    if (current_email === new_email) {
      return res.status(400).json({ error: "New email address must be different from current email address" });
    }
    ...
    conn.query(checkUserQuery, [new_email], (err, result) => {
      ...
      conn.query(checkVerificationCodeQuery, [current_email, verification_code], (err, result) => {
        if (err) {
          console.error(err);
          return res.status(500).json({ error: "Internal server error" });
        }
        ...
        const updateUserQuery = `UPDATE users SET email = ? WHERE email = ?`;
        conn.query(updateUserQuery, [new_email, current_email], (err, result) => {
          if (err) {
            console.error(err);
            return res.status(500).json({ error: "Internal server error" });
          }
          ...
          conn.query(deleteVerificationCodeQuery, [current_email, verification_code], (err, result) => {
            if (err) {
              console.error(err);
              return res.status(500).json({ error: "Internal server error" });
            }
  ...
  conn.query(checkUserQuery, [email], (err, result) => {
    ...
    conn.query(insertTokenQuery, [email, token], (err, result) => {
      ...
      const mailOptions = {
        ...
        to: email,
        subject: "Reset your password",
        html: `<p>Please click this link to reset your password:</p><br><a href="${process.env.CLIENT_URL_CHANGE_PASSWORD}">Reset Password</a>`,
      };
      transporter.sendMail(mailOptions, (err, info) => {
        if (err) {
          console.error(err);
        ...
        res.status(200).json({ message: "Password reset link has been sent to your email address and change password link will expire in 1 hour" });
      });
    });
  });
};
...
const reset_password = async (req, res) => {
  ...
  conn.query(checkTokenQuery, [token], (err, result) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
      ...
      // Update the user's password in the database
      const updatePasswordQuery = `UPDATE users SET password = ? WHERE email = ?`;
      conn.query(updatePasswordQuery, [hashedPassword, result[0].email], (err, result) => {
        if (err) {
          console.error(err);
          return res.status(500).json({ error: "Internal server error" });
        }
      ...
      try {
        ...
        conn.query(selectUserQuery, [profile.emails[0].value], (err, result) => {
          ...
          if (result.length === 0) {
            ...
            conn.query(insertUserQuery, [profile.displayName, profile.emails[0].value, profile.id], (err, result) => {
              if (err) {
                return done(err);
              }
              const user = { id: result.insertId, name: profile.displayName, email: profile.emails[0].value, google_id: profile.id };
              const token = jwt.sign(user, process.env.JWT_SECRET, { expiresIn: "1h" });
              return done(null, { token });
            });
          } else {
            ...
            conn.query(updateUserQuery, [profile.id, profile.emails[0].value], (err, result) => {
              if (err) {
                return done(err);
              }
              const user = { id: result.insertId, name: profile.displayName, email: profile.emails[0].value, google_id: profile.id };
...
const facebook_callback = (req, res) => {
  passport.authenticate("facebook", { session: false }, (err, user) => {
    ...
    res.cookie("token", user.token, { httpOnly: true, secure: process.env.NODE_ENV === "production" });
    res.redirect("http://localhost:8000/welcome-facebook");
  })(req, res);
};
